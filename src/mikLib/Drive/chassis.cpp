...
static double degToInches(double deg) {
    double wheel_circum = M_PI * 2.75;
    return (deg / 360.0) * wheel_circum * 0.75;
}

/*// Average distance of both sides in INCHES
float Chassis::get_ForwardTracker_position() {
    // These calls depend on mik::motor_group API.
    // If motor_group::position uses degrees for the whole group, this is enough:
    double left_deg  = left_drive.position(degrees);
    double right_deg = right_drive.position(degrees);

    double avg_deg = (left_deg + right_deg) / 2.0;
    return degToInches(avg_deg);
}

*/
Chassis::Chassis(mik::motor_group left_drive, mik::motor_group right_drive, int inertial_port, float inertial_scale, float wheel_ratio, float wheel_diameter, int forward_tracker_port, float forward_tracker_diameter, 
...
float Chassis::get_MotorEncoder_position() {
   return ((chassis.left_drive.position(vex::deg) + chassis.right_drive.position(vex::deg)) / 2.0)  * drive_in_to_deg_ratio;
}
float Chassis::get_ForwardTracker_position() {
    //return forward_tracker.position(vex::deg) * forward_tracker_inch_to_deg_ratio;
    return get_MotorEncoder_position();
}
